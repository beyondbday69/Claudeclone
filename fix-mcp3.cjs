...
// The modal content starts after <div className="flex items-center justify-between px-5 py-3.5 border-b border-app-border/20"> and some other divs.
// I will just replace the modal content by using regex on {!(mcpConnected || demoMcpConnected) ? ( ... )}
const modalRegex = /\{!\(mcpConnected \|\| demoMcpConnected\) \? \(([\s\S]*?)<\/div>\s*\}\s*<\/div>\s*<\/div>\s*\)\}/;

const newModalBody = `{mcpServers.length === 0 ? (
                 ...
                 <div className="p-5 flex flex-col gap-5">
                    ...
                    <div className="space-y-2">
                      ...
                      {mcpServers.map(url => (
                        <div key={url} className="flex items-center justify-between p-2.5 bg-app-surface/20 border border-app-border/20 rounded font-mono text-xs text-app-textSecondary mb-2">
...
code = code.replace(modalRegex, newModalBody);
