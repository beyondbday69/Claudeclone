...
const oldGetMcpName = `const getMcpName = () => {
    try {
...
const newGetMcpName = `const getMcpName = (urlStr: string) => {
    try {
...
code = code.replace(oldGetMcpName, newGetMcpName);

// For the modal, let's just find the exact string that starts the modal body and ends the file
const startOfModalBody = `<div className="flex items-center justify-between px-5 py-3.5 border-b border-app-border/20">
...
const startIndex = code.indexOf(startOfModalBody);
if (startIndex !== -1) {
    const endIndex = code.lastIndexOf(endOfFile);
    if (endIndex !== -1 && endIndex > startIndex) {
        const replacement = startOfModalBody + `
