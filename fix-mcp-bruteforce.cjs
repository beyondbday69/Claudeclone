...
// 1. Fix getMcpName
code = code.replace(/const getMcpName = \(\) => \{[\s\S]*?catch \(e\) \{ return "MCP Server"; \}\n  \};\n/, `const getMcpName = (urlStr: string) => {\n    try {\n      const url = new URL(urlStr);\n      const hostPart = url.hostname.split('.')[0];\n      return hostPart.charAt(0).toUpperCase() + hostPart.slice(1);\n    } catch (e) { return "MCP Server"; }\n  };\n`);

// 2. Fix the entire modal block.
// We'll replace from `{showMcpDialog && (` all the way to `</main>`
const modalStartStr = '{showMcpDialog && (';
const mainEndStr = '</main>';
const startIndex = code.indexOf(modalStartStr);
const endIndex = code.lastIndexOf(mainEndStr);
...
code = code.substring(0, startIndex) + newModalStr;
